...
def est_extract():
    ...
    for d in range (1,4):
        for e in range (1,4):
            message_bit_array = est_extract_bits_from_image(image_byte_array, header_len, d,e)
            est = est_bit_proportion(message_bit_array)
            print(" p = " + str(d) + " " + " q = " + str(e) + " " +" bp= " + str("%.3f" %est))
        if est<=0.5:
            p=d
            q=e
    print("The answer is: p = " + " " + str(p)+ " " + " q = " + str(q))

def est_bit_proportion (values):
     count= 0
     for i in values:
        if i == 1:
            count = count + 1
